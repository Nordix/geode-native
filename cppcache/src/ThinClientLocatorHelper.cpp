...
ThinClientLocatorHelper::createConnection(
  ...
  if (sys_prop.sslEnabled()) {
    conn = new TcpSslConn(hostname.c_str(), port, timeout, buffer_size,
                          sys_prop.sslTrustStore().c_str(),
                          sys_prop.sslKeyStore().c_str(),
                          sys_prop.sslKeystorePassword().c_str());
  } else {
    conn = new TcpConn(hostname.c_str(), port, timeout, buffer_size);
  }
