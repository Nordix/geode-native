...
void runClientOperations(std::shared_ptr<Cache> cache,
  ...
  EXPECT_TRUE(region->containsKey(7));
}
*/
void executeTestCaseOneTransaction() {
  Cluster cluster{
  ...
  // std::thread th(runClientOperations, cache, region);
  ...
  */
}  // executeTestCaseOneTransaction

TEST(DisconnectEndPointAtException, useOneTransaction) {
  executeTestCaseOneTransaction();
}
